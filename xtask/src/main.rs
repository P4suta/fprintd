...
//!
//! That is not only a matter of taste here. The tasks this crate replaced could not run at all on
//! the project's own dev box: they carried `MSYS_NO_PATHCONV=1` and `$(pwd -W 2>/dev/null || pwd)`
//! to survive Git Bash rewriting a bind-mount path, and mise runs tasks through `cmd.exe`, which
//! understands neither. See [`docker`].
//!
