...
//!
//! Hand-rolled in the same spirit as `fp-core`'s error type (no `thiserror`, no upstream
...
/// Everything that can go wrong turning bytes into a [`Print`](fp_core::Print) or back.
///
...
pub enum Fp3Error {
    ...
    /// is never serialized).
    UnknownType(i32),
    /// Asked to serialize a [`Template::Undefined`](fp_core::Template::Undefined) print —
