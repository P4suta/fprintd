...
//! on-disk `"FP3"` blob and the pure [`fp_core::Print`] domain model.
//!
...
//! NBIS `(a(aiaiai))` payload — lives here and never leaks up into `fp-core`
...
//! ```no_run
//! use fp_core::Print;
//! # fn demo(print: &Print, blob: &[u8]) -> fp_fp3::Result<()> {
//! let bytes: Vec<u8> = fp_fp3::to_bytes(print)?;
//! let print: Print = fp_fp3::from_bytes(blob)?;
//! # let _ = bytes; let _ = print; Ok(())
//! # }
//! ```
//!
//! For every [`Print`](fp_core::Print) this crate can serialize, `from_bytes(to_bytes(p))`
