...
//! (`fp-backend-libfprint`, `FP3_FREEZE_FIXTURES=1`, which also asserts byte-identity live). This
//! test pins the interop the other way — on any platform, with no Docker — proving `fp-fp3` decodes
...
fn decodes_and_reencodes_real_libfprint_fp3_byte_for_byte() {
    ...
    let print = fp_fp3::from_bytes(blob).expect("fp-fp3 must decode real libfprint FP3");

    // ...bound to the virtual_device driver, carrying an opaque Raw (driver-blob) template.
    assert!(
        blob.starts_with(fp_fp3::MAGIC),
    ...
    let reencoded = fp_fp3::to_bytes(&print).expect("re-encode");
    assert_eq!(
        reencoded.as_slice(),
        blob.as_slice(),
        "fp-fp3 must reproduce libfprint's FP3 bytes exactly"
    );
}
