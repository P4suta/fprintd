...
//! serialize the resulting [`Print`](fp_core::Print) to the FP3 on-disk blob, read it back,
//! and confirm the deserialized print still verifies against the sensor.
//!
//! This is the one test that exercises all three crates at once — `fp-backend-native`
//! (a [`fp_core::Device`]), `fp-fp3` (the wire codec), and `fp-core` (the domain model they
//! meet in). It sits deliberately in `fp-fp3`, which is above the domain model and so may
...
use fp_core::{Device, Finger, Print, Template};
...
fn host_enroll_fp3_roundtrip_then_verify() {
    ...
    let bytes = fp_fp3::to_bytes(&print).unwrap();
    assert!(bytes.starts_with(fp_fp3::MAGIC));
    let back = fp_fp3::from_bytes(&bytes).unwrap();
