...
//!
//! This ties three crates together the way a live host-image driver would: `fp-core` (the domain
//! `Print`/`Template::Nbis`), `fp-fp3` (the wire codec), and `fp-bozorth3` (the matcher, reached via
//! `fp-backend-native`'s `nbis_match_score` seam). It exercises what the virtual device's
...
fn nbis_print(samples: Vec<Vec<Minutia>>) -> Print {
    Print {
        ...
        finger: Some(fp_core::Finger::LeftIndex),
        driver: Some(fp_core::DriverId("virtual_image".into())),
        device_id: Some(fp_core::DeviceId("seam-test".into())),
        ..Print::default()
    }
}
...
fn enrolled_nbis_print_survives_fp3_and_matches_via_bozorth3() {
    ...
    let bytes = fp_fp3::to_bytes(&enrolled).expect("serialize FP3");
    assert!(bytes.starts_with(fp_fp3::MAGIC));
    let back = fp_fp3::from_bytes(&bytes).expect("deserialize FP3");
