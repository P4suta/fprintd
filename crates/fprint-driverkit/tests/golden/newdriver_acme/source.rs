...
//! [`AcmeFrameSource`]: a [`crate::FrameSource`] driven over any [`UsbTransport`].
//!
//! `ImageDevice<AcmeFrameSource<NusbTransport>>` runs the host-image pipeline over a Acme
//! sensor with no change to `ImageDevice`. Being generic over the transport, the same driver runs
//! against a scripted mock offline, so the protocol → transport → frame-assembly path is exercised on
...
//! whether the transport is genuinely async or resolves immediately.
...
/// A host-image capture source that speaks the Acme protocol over a [`UsbTransport`].
pub struct AcmeFrameSource<T: UsbTransport> {
    transport: T,
...
impl<T: UsbTransport> AcmeFrameSource<T> {
    /// Build a Acme driver over `transport`, using this driver's documented endpoints and scan
    /// resolution (see the [`acme`](super::acme) module — several of those values are
    /// HW-verification placeholders).
    pub fn new(transport: T) -> Self {
        AcmeFrameSource {
            transport,
            ep_out: super::acme::EP_OUT,
            ep_in: super::acme::EP_IN,
            ppi: super::acme::PPI,
        }
    }
    ...
    ///
    /// A handshake only ever writes to the device, so a [`UsbTransfer::BulkIn`] here is a construction
    /// bug in the sequence, not a wire condition — the arm/disarm sequences are this driver's own
    /// [`acme`](super::acme) data and never contain one.
...
impl<T: UsbTransport> FrameSource for AcmeFrameSource<T> {
    async fn capture(&mut self) -> Result<Capture> {
        ...
        if count > super::acme::MAX_FRAME_BYTES {
            return Err(Error::Protocol(format!(
                "frame header claims {count} bytes, over the {} maximum",
                super::acme::MAX_FRAME_BYTES
            )));
        }
    ...
    async fn arm(&mut self) -> Result<()> {
        for step in super::acme::init_sequence() {
    ...
    async fn disarm(&mut self) -> Result<()> {
        for step in super::acme::deinit_sequence() {
            self.run_step(&step).await?;
        }
        Ok(())
    }
