...
//! Pure USB framing for the Acme host-image driver: `Vec<u8>` in, `Vec<u8>` / [`Frame`] out.
//!
//! This module has **no transport and no `nusb`**: every function is a pure transformation of byte
//! slices, so the whole framing layer is unit-tested on any platform. The transport moves the bytes;
//! this module decides what they mean.
//!
//! ## Provenance
//!
//! The wire shapes here are **original code describing interoperability facts** (a header layout, the
//! image geometry, a capture opcode) — not a transliteration of any libfprint driver. Where a real
//! byte value has not been confirmed against a physical Acme sensor it is a documented
//! placeholder marked `HW-verified: required`, so nothing here is asserted as fact that has not been
//! observed.
...
///
/// HW-verified: required. Placeholder framing chosen so the header is self-describing and
/// round-trippable in tests; the real Acme image-stream delimiting must be confirmed on hardware.
...
///
/// HW-verified: required. Placeholder opcode; the real capture request byte(s) for the Acme come
...
///
/// Kept as a function (not a `const`) because a real driver's capture request often carries a
...
/// scripted transport, which lifts recorded device bytes back onto the wire.
#[must_use]
...
mod tests {
    ...
    fn assemble_frame_rejects_length_mismatch() {
        let a = [1u8, 2, 3];
        assert!(assemble_frame(&[&a], 4, 2, 500).is_err());
    }
}
