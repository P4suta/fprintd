...
//!
//! * `fprint-backend-libfprint` can implement [`Backend`] by wrapping the C libfprint (the
//!   M1 shim), and
//! * `fprint-backend-native` can implement the same trait with pure-Rust drivers + transport,
//!
//! and the fprintd-compatible daemon (`fprintd`) depends only on these traits, so the
