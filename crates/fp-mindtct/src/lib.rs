...
//! dependency on the domain model. A consumer (e.g. `fp-backend-native`) converts to its
//! `fp_core::Minutia` at the boundary.
...
/// (higher is better). Mirrors the shape of `fp_core::Minutia` (the consumer converts).
