...
//! # fp-backend-libfprint — the M1 shim
//!
//! A [`fp_core::Backend`]/[`fp_core::Device`] implementation that wraps the **C libfprint**
...
//! `fp-core` never requires `Send`, exactly so this backend can be honest about its thread
//! affinity (principle 7); the daemon confines each device to a single actor thread.
//!
//! ## The honest cancellation limitation
//!
//! `fp-core`'s contract is "**dropping the future cancels the operation**" (principle 4).
...
//! belongs to `fp-backend-native`; here cancellation is best-effort and only meaningful when
