...
//! generic over any [`fp_core::Backend`]: the `fprintd` binary drives the libfprint shim
//! ([`fp_backend_libfprint`]); the integration test drives `fp-backend-native`'s virtual
...
//!   `!Send` `fp_core::Device`; the D-Bus objects talk to it over `Send` channels
...
//!   `/var/lib/fprint` layout in [`storage`], the FP3 bytes in [`fp_fp3`].
//!
...
use fp_core::{Backend, Device as _, DeviceInfo};
...
pub fn run() {
    ...
    if let Err(e) = runtime.block_on(serve(test_mode)) {
        tracing::error!("fprintd-rs fatal: {e:?}");
        std::process::exit(1);
    }
}
...
async fn serve(test_mode: bool) -> Result<(), DaemonError> {
    ...
    let factory = || fp_backend_libfprint::LibfprintBackend::new();

    let daemon = Daemon::new(factory, authz);
    let _connection = daemon.serve_system().await?;
    tracing::info!("fprintd-rs serving {BUS_NAME}");

    wait_for_shutdown().await;
    tracing::info!("fprintd-rs shutting down");
    Ok(())
}
