...
//!
//! The libfprint shim is thread-affine (GObject / `GMainContext`), so `fp_core::Device` is
...
//! the actor runs the matching `fp_core` async method on its own thread and answers via the
...
use fp_core::{Backend, Device, DeviceId, DeviceInfo, EnrollProgress, Error};
use tokio::sync::mpsc;
