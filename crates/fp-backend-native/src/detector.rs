...
//! [`fp_core::Minutia`], so a driver can go **image → minutiae → match** end-to-end.
//!
//! The conversion lives here (not in `fp-mindtct`) on purpose. `fp-mindtct` is a self-contained,
//! dependency-free public-domain kernel that does not know `fp-core`; the only thing crossing the
//! boundary is the `xyt` triple — an interoperability *fact*, not a code coupling. Keeping the
//! `Minutia → fp_core::Minutia` mapping on the permissive (`fp-backend-native`) side of the fence is
//! what lets the PD detector and PD matcher stay pristine, each defining its own `Minutia`.

use fp_core::{Minutia, Template};

/// Detect the minutiae in a captured 8-bit grayscale frame, as [`fp_core::Minutia`]s.
///
/// Runs the full MINDTCT pipeline ([`fp_mindtct::detect_minutiae`]) over `img` and projects each
...
/// `quality` is dropped, because `fp_core::Minutia` (like libfprint's `xyt_struct` for BOZORTH3)
/// does not carry it. An image with no detectable ridge structure yields an empty vector.
#[must_use]
pub fn extract_minutiae(img: fp_mindtct::GrayImage<'_>) -> Vec<Minutia> {
    fp_mindtct::detect_minutiae(img)
        .iter()
        .map(to_core)
        .collect()
}

/// Build a host-side [`fp_core::Template::Nbis`] from a set of captured frames.
///
...
#[must_use]
pub fn template_from_images(images: &[fp_mindtct::GrayImage<'_>]) -> Template {
...
#[inline]
fn to_core(m: &fp_mindtct::Minutia) -> Minutia {
    Minutia {
        x: m.x,
        y: m.y,
        theta: m.theta,
