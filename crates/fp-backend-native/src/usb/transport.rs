...
///
/// `async fn` in this trait is deliberate and mirrors [`fp_core::Device`] and
...
impl UsbTransport for NusbTransport {
    async fn bulk_out(&mut self, ep: u8, data: &[u8]) -> Result<()> {
        ...
        completion
            .into_result()
            .map(|_| ())
            .map_err(|e| fp_core::Error::Transport(format!("bulk_out ep {ep:#04x}: {e}")))
    }
    ...
    async fn bulk_in(&mut self, ep: u8, len: usize) -> Result<Vec<u8>> {
        ...
        completion
            .into_result()
            .map_err(|e| fp_core::Error::Transport(format!("bulk_in ep {ep:#04x}: {e}")))
    }
    ...
    async fn control(
        ...
        if request_type & 0x80 != 0 {
            ...
            let length = u16::try_from(data.len()).map_err(|_| {
                fp_core::Error::Transport("control IN length exceeds u16".to_string())
            })?;
            let completion = self
                .iface
            ...
            completion.into_result().map_err(|e| {
                fp_core::Error::Transport(format!("control_in req {request:#04x}: {e}"))
            })
        } else {
            let completion = self
                .iface
            ...
            completion.into_result().map(|_| Vec::new()).map_err(|e| {
                fp_core::Error::Transport(format!("control_out req {request:#04x}: {e}"))
            })
        }
    }
}
