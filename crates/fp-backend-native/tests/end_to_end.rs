...
//!
//! This closes the host-image loop the two seams model: [`fp_backend_native::template_from_images`]
//! runs the real MINDTCT detector ([`fp_mindtct`]) over a synthetic fingerprint frame, converts the
//! detected minutiae into an [`fp_core::Template::Nbis`], and
//! [`fp_backend_native::nbis_match_score`] scores it with the real BOZORTH3 matcher
//! ([`fp_bozorth3`]). No `.raw` fixtures are read: the images are generated in-process by a tiny LCG
...
use fp_backend_native::{extract_minutiae, nbis_match_score, template_from_images};
use fp_core::Template;
use fp_mindtct::GrayImage;
