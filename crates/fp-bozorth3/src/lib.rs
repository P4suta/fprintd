...
//! ## Provenance
//!
//! BOZORTH3 is public-domain U.S. Government software (title 17 §105). This crate is written from
//! the **stock upstream NBIS** algorithm (see `docs/bozorth3-algorithm.md`), verified black-box
//! against the stock C tool, and quarantined under SPDX `LicenseRef-NBIS-PD` so the project's
//! permissive core stays clean — it is deliberately **not** derived from libfprint's patched
//! `nbis/` copy (whose changes carry LGPL terms). See `ARCHITECTURE.md` §Provenance & licensing.
//!
//! ## Shape
//!
...
//! model. A consumer (e.g. `fp-backend-native`) converts its `fp_core::Minutia` at the boundary.
//!
//! ```
//! use fp_bozorth3::Minutia;
//! let probe = [Minutia { x: 10, y: 20, theta: 90 } /* … */];
//! let gallery = [Minutia { x: 11, y: 19, theta: 92 } /* … */];
//! let score = fp_bozorth3::match_score(&probe, &gallery);
...
/// `0..=359` internally. Matches the shape of `fp_core::Minutia` (the consumer converts).
